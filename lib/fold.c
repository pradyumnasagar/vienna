...
PUBLIC int   eos_debug=1;  /* verbose info from energy_of_struct *///AO
...
PRIVATE int stack_energy(int i, const char *string)  
{
  ...
  if (p>q) {                       /* hair pin */
	...
	//     i, j, string[i-1],string[j-1], ee);
     printf("Hairpin  loop (%3d ,%3d ) %c%c              : %5d\n",
	     i,j,string[i-1],string[j-1], ee);//HairpinE(i, j, type, string));
  ...
  //	   i,j,string[i-1],string[j-1],ee);
  printf("Multi    loop (%3d,%3d) %c%c              : %5d\n",
	   i,j,string[i-1],string[j-1],ee);//ML_Energy(i));
