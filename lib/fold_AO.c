...
//PRIVATE int   stack_energy(int i, char *string, FILE *f);AO
...
PRIVATE int   eos_debug=1; /*MR11012005*/
...
float fold(char *string, char *structure)
{
  ...
  energy_of_struct(string,structure); /*MR11012005*/
...
float energy_of_struct(char *string, char *structure)
{
  ...
  free(S); free(S1);
  S=ss; S1=ss1;
  //printf("in energy_of_struct %s\n%s\n %d\n",string,structure,energy);
...
int energy_of_struct_pt(char *string, short * ptable, short *s, short *s1) {
  ...
  //FILE *f;//AO
  //f=fopen("PairingDetailsTrainPosEnFiltHmmFilt-f-90.txt","a");//AO
  energy=lastd=ld3=0;
  pair_table = ptable;
  S = s;
  S1 = s1;
  ...
  for (i=1; i<=length; i++) {
    ...
    //if(i == 1){
	//fprintf(f,"%dFrom the one\n", myIndex);
  	//myIndex = myIndex+1;
	//}
    //energy += stack_energy(i, string,f);AO
    energy += stack_energy(i, string);

    //    printf("%03d %c %d\n",i,string[i],stack_energy(i, string));/*MR11012005*/

    if (backtrack_type=='M') energy+=MLintern[pair[S[i]][S[pair_table[i]]]];
    i=pair_table[i];
    //fprintf(f,"BackTracking\n");
  }
...
/*---------------------------------------------------------------------------*/
PRIVATE int stack_energy(int i, char *string)//AO//,FILE *f
{
  ...
  while (p<q) { /* process all stacks and interior loops */
    ...
    //Print All to file AO
    //fprintf(f,"Interior loop (%3d,%3d) %c%c; (%3d,%3d) %c%c: %5d\n",
    //i,j,string[i-1],string[j-1],p,q,string[p-1],string[q-1], ee);
    printf("Interior loop (%3d ,%3d ) %c%c; (%3d ,%3d ) %c%c: %5d\n",
	   i,j,string[i-1],string[j-1],p,q,string[p-1],string[q-1], ee);
  } /* end while */
  ...
  if (p>q) {                       /* hair pin */
    ...
    if (eos_debug)
      ...
      //printf("Hairpin  loop (%3d,%3d) %c%c              : %5d\n",
	     ...
	     //fprintf(f,"Hairpin  loop (%3d,%3d) %c%c              : %5d\n",
	     //i,j,string[i-1],string[j-1],HairpinE(i, j, type, string));
      printf("Hairpin  loop (%3d ,%3d ) %c%c              : %5d\n",
	     i,j,string[i-1],string[j-1],HairpinE(i, j, type, string));
    //fclose(f);
  ...
  while (p<j) {
    ...
    //energy += stack_energy(p, string,f);
  ...
  if (eos_debug)
    //printf("Multi    loop (%3d,%3d) %c%c              : %5d\n",
	   ...
	   //fprintf(f,"Multi    loop (%3d,%3d) %c%c              : %5d\n",
	   //i,j,string[i-1],string[j-1],ML_Energy(i));
  printf("Multi    loop (%3d,%3d) %c%c              : %5d\n",
	   i,j,string[i-1],string[j-1],ML_Energy(i));
  //fclose(f);
